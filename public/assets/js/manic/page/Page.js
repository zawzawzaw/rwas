...
goog.require('manic.ui.NewFormCheck');
...
manic.page.Page = function(options) {
  ...
  /**
   * @type {Array.<manic.page.Section>}
   */
  this.page_section_array = [];
  ...
  /**
   * @type {Array.<manic.ui.NewFormCheck>}
   */
  this.form_check_02_array = [];
...
manic.page.Page.prototype.init = function(){
  ...
  this.controller = new ScrollMagic.Controller(); // needed by some components       // updated
  ...
  this.smooth_scrollbar = Scrollbar.init($('#page-wrapper')[0], {
    speed: 1,

  });
  this.smooth_scrollbar.addListener(function(event){
    console.log('testing');
  }.bind(this));
  */
  ...
  this.on_window_resize_initial();
  this.hide_preloader();

  this.window.on('hashchange', this.on_window_hash_change.bind(this));
  // this.on_window_hash_change(null);
  this.on_window_hash_change_instant();


  this.update_page_layout();                      // take this out if you have one somewhere else
...
/**
 * @param {manic.page.Section} param
 */
manic.page.Page.prototype.add_page_section = function(param) {

  // manic.page.Section
  
  // should i check ? 
  param.page = this;
  this.page_section_array[this.page_section_array.length] = param;
};
...
manic.page.Page.prototype.hide_preloader = function() {
  ...
  // TweenMax.to(this.preloader_element, 0.3, {autoAlpha:0, delay: 0.1, ease:Linear.easeNone});
  // TweenMax.to(this.preloader_element, 0.3, {autoAlpha:0, delay: 0.3, ease:Linear.easeNone});  // too slow
  
    
  TweenMax.to(this.preloader_element, 0.3, {autoAlpha:0, delay: 0.3, ease:Linear.easeNone});
 ...
 * @param  {String=} str_param_3
 */
manic.page.Page.prototype.scroll_to_target_instant = function(str_param, str_param_2, str_param_3) {
  ...
  for (var i = 0, l = this.scrolltarget_name_array.length; i < l; i++) {
    ...
    if (str_param == scroll_target_str) {
      ...
      // console.log('manic.page.Page: scroll_to_target: ');
      // console.log(scroll_target);
      // this.controller.scrollTo(scroll_target[0]);                      // just replaced this

      var target = scroll_target[0];
...
// this isn't used on hash... used somewhere else..
...
manic.page.Page.prototype.scroll_to = function(num_param) {
  ...
  var target_duration = Math.abs( (target_y - current_scroll) / 800 );
  // console.log('target_duration: ' + target_duration);
...
manic.page.Page.prototype.create_image_container = function() {
  ...
  for (var i = 0, l=arr.length; i < l; i++) {
    ...
    temp_item.addClass('instantiated-version');
...
manic.page.Page.prototype.create_video_container = function(){
  ...
  for (var i = 0, l=arr.length; i < l; i++) {
    temp_item = $(arr[i]);
    video_container = new manic.ui.VideoContainer({}, temp_item);
  }
...
manic.page.Page.prototype.create_animated_svg = function() {
  ...
  for (var i = 0, l=arr.length; i < l; i++) {
    temp_item = $(arr[i]);
    animated_svg = new manic.ui.AnimatedSVG({}, temp_item);
  }
...
manic.page.Page.prototype.create_expand_container = function(){
  ...
  for (var i = 0, l=arr.length; i < l; i++) {
    temp_item = $(arr[i]);
    expand_container = new manic.ui.ExpandContainer({}, temp_item);
...
manic.page.Page.prototype.create_dropdown = function() {
  ...
  for (var i = 0, l = arr.length ; i < l; i++) {
    temp_item = $(arr[i]);
    item_id = '' + temp_item.find('select').attr('id');

    dropdown = new manic.ui.Dropdown({}, temp_item);
...
manic.page.Page.prototype.create_form_check = function() {
  ...
  for (var i = 0, l = arr.length ; i < l; i++) {
    ...
    form_check = new manic.ui.NewFormCheck({}, temp_item);
    this.form_check_02_array[this.form_check_02_array.length] = form_check;
  }
...
manic.page.Page.prototype.on_window_resize_initial = function(event) {
...
manic.page.Page.prototype.on_expand_container_expand = function(event) {
  ...
  if (manic.IS_MOBILE == true) {
    ...
    if (expand_container.scroll_value != 'none'){
      ...
      // this.util_scroll_to(expand_container.scroll_value);                            // original behavior
...
manic.page.Page.prototype.update_page_layout = function(){
  ...
  /**
   * @type {manic.page.Section}
   */
  var page_section = null;

  for (var i = 0, l=this.page_section_array.length; i < l; i++) {
    page_section = this.page_section_array[i];
    page_section.update_section_layout();
  }
...
manic.page.Page.prototype.update_manic_image_containers = function(){
  ...
  for (var i = 0, l=this.manic_image_array.length; i < l; i++) {
    temp_item = this.manic_image_array[i];
    temp_item.update_layout();
  }
};
...
manic.page.Page.prototype.controller_scroll_to = function(target) {
  ...
  var target_y = target;
  var current_scroll = this.window.scrollTop();
  var target_duration = Math.abs( (target_y - current_scroll) / 1200 );
  target_duration *= 0.6;
  // target_duration *= 0.7;

  target_duration = target_duration < 0.8 ? 0.8 : target_duration;

  // console.log('target_duration: ' + target_duration);

  TweenMax.to(window, target_duration, {
    scrollTo : {
      y : target, // scroll position of the target along y axis
      // autoKill : true // allows user to kill scroll action smoothly
...
// copied from on_window_hash_change

manic.page.Page.prototype.on_window_hash_change_instant = function() {
  ...
  if (this.window_hash != '') {
    ...
    if(str_array.length == 1){
      ...
      this.scroll_to_target_instant(this.window_hash_01);
      ...
      this.scroll_to_target_instant(this.window_hash_01, this.window_hash_02);
      ...
      this.scroll_to_target_instant(this.window_hash_01, this.window_hash_02, this.window_hash_03);
