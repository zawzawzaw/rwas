...
manic.ui.ComboBox.prototype.set_option_array = function(arr_param) {
  ...
  for (var i = 0, l=arr_param.length; i < l; i++) {
    temp_item = arr_param[i];

    if (goog.isDefAndNotNull(temp_item['label']) && goog.isDefAndNotNull(temp_item['value'])) {
      this.label_array[i] = temp_item['label'];
      this.value_array[i] = temp_item['value'];

      menu_item = new goog.ui.ComboBoxItem(temp_item['label']);
      this.goog_select.addItem(menu_item);
    }
  }
