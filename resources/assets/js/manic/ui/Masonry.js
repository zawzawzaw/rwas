...
manic.ui.Masonry.prototype.hide_not_visible = function() {
  ...
  for (var i = 0, l = arr.length; i < l; i++) {
    temp_item = $(arr[i]);
    TweenMax.killTweensOf(temp_item);
    //TweenMax.to(temp_item,0.8,{autoAlpha:0,width:0,height:0,marginRight:0,marginBottom:0});
    TweenMax.to(temp_item, 0.4, {
  ...
  for (var r = 0; r < row_num; r++) {
    ...
    for (var c = 0; c < col_num; c++) {
      ...
      if (i < arr.length) {
        ...
        //if(temp_item.hasClass('ui-sortable-helper') == false){
          w = parseInt(temp_item.attr('data-width'));
          h = parseInt(temp_item.attr('data-height'));
          temp_item.data('w', w);
          temp_item.data('h', h);
          col_array[col_array.length] = temp_item;
    ...
    for (var c = 0, clll = col_array.length; c < clll; c++) {
      ...
      TweenMax.killTweensOf(temp_item);
      //TweenMax.to(temp_item,0.4,{
      TweenMax.to(temp_item, temp_duration, {
        autoAlpha: 1,
        width: target_width,
